...
3  열람 루프
     ...
     d <분야>   → 분야별
     s <검색어> → 이름·기능으로 찾기
     g          → 1단계로 되돌아가 목표를 다시 고른다 (추천이 다시 계산된다)
     r          → 추천 다시 보기
     p          → 지금까지 담은 것
     ?          → 카탈로그 보는 법
     h          → 메뉴 다시 보기
...
목표 선택(1단계)과 추천(2단계)은 열람 루프에서 `g` / `r` 로 언제든 다시 부를 수 있다.

`g` 는 지금 고른 목표를 먼저 보여주고, 엔터만 치면 바꾸지 않는다.
목표를 바꾸면 추천은 **다시 계산된다** (누적되지 않는다).
이미 담은 결정은 유지된다 — 목표를 바꿨다고 고른 것이 사라지면 안 된다.

### 3.6 비대화 환경에서 매달리지 않는다

`Read-Host` 는 EOF 에서 **빈 문자열이 아니라 `$null`** 을 준다.
그것을 문자열로 다루면 `.Trim()` 에서 터진다. 실제로 터졌다 —
AI CLI 안에서 `! powershell ... Invoke-HarnessEscort.ps1` 로 부르면 입력이 연결되지 않는다.

`$null` 은 "입력이 없다"가 아니라 **"입력 통로가 닫혔다"**는 뜻이므로 즉시 종료 코드 `3` 으로 빠지고,
`-List` / `-Explain` / `-Tips` 를 안내한다. 빈 줄이 3번 연속인 경우도 같이 처리한다.

CI 나 AI 세션에서 실수로 부르면 조용히 멈춰 있거나 터지는 대신 즉시 알린다.
