...
**미결 항목은 1건이며 그것도 보류로 결정됐다.**
- 레거시 `C:\AI-Tools` 삭제(224MB) — **보류.** 옛 agy 세션(pid 5996)이 그 경로에서
  MCP 서버를 띄운 채 살아 있다(§5-45). 그 세션이 끝나면 바로 지울 수 있다
- 중복 claude.ai Google 커넥터 — **그대로 두기로 결정됐다.** 다시 제안하지 않는다(§9.1-6)
...
| 7 | **레거시 `C:\AI-Tools` 삭제** (224MB) | 그 경로에서 뜬 프로세스를 먼저 끝내야 한다 | **보류 (2026-08-23 사용자 결정: 나중에).** 옛 agy 세션(pid 5996)이 레거시 래퍼로 MCP 서버를 띄운 채 살아 있다 (§5-45). 그 세션이 끝나면 바로 지울 수 있다 |
