...
- **클라이언트 등록(codex 등) — 미결이 아니다** (2026-08-24 추가, §7-2).
  Layer B 는 쓰는 사람이 쓸 때 연결한다. 진단이 `unregistered` 를 보고해도 할 일이 아니다
...
| ~~2~~ | ~~Codex 등록~~ | **2026-08-24 종결. 미결 사항이 아니다.** argv 순서 자체는 통한다(§5-43). 2026-08-24 확인 시 실제로 소실돼 있었다(orca 가 `config.toml` 을 소유한다 — §5-10). **그래도 다시 제안하지 않는다.** 클라이언트 등록은 Layer B 이고 **쓰는 사람이 쓸 때 직접 연결하는 것**이다. 저장소가 특정 PC 의 등록 상태를 미결로 들고 있을 이유가 없다. 진단이 `codex unregistered` 를 계속 보고하겠지만 그것은 **정상 상태의 보고**이지 할 일이 아니다 |
